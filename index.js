...
module.exports = function(options){
  var localExports = {},
      templateFinder = require('./shared/templateFinder')(Underscore);

  /**
   * Export the `Underscore` object, so other modules can add helpers, partials, etc.
   */
  localExports.Underscore = Underscore;
  ...
  if (typeof window === 'undefined') {
    ...
    localExports.getLayout = require(serverOnlyLayoutFinderPath)(Underscore).getLayout;
  } else {
  ...
  localExports.registerHelpers = function registerHelpers(helpersModule) {
    var helpers = helpersModule(Underscore, localExports.getTemplate);

    for (var key in helpers) {
      if (!helpers.hasOwnProperty(key)) continue;
      Underscore.extend(key, helpers[key]);
    }
  };
